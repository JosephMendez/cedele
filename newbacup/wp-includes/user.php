 ...
 *                      be created.
 */
function wp_insert_user( $userdata, $sendEmail = true ) {
	global $wpdb;
	...
	if ( $update ) {
        ...
        $data['edenred_id'] = isset($userdata['edenred_id']) ? $userdata['edenred_id'] : null;
	...
	if ( $update ) {
		...
		if ( isset( $userdata['spam'] ) && $userdata['spam'] != $old_user_data->spam ) {
			if ( 1 == $userdata['spam'] ) {
				 ...
				 * @since 3.0.0
				 *
				 * @param int $user_id ID of the user marked as HAM.q
				 */
