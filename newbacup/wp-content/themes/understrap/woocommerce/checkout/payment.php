...
?>
<div id="payment" class="woocommerce-checkout-payment <?= Rewards_Product_Exits() ? 'woocommerce-checkout-payment__membership':null ?>">
	<?php if ( WC()->cart->needs_payment() ) : ?>
		<h3>Payment Method</h3>
		<p class="payment-instruction highlight">Please make sure to fill in the recipient’s information</p>
		<p class="payment-instruction">You cannot change order after payment</p>
		<ul class="wc_payment_methods payment_methods methods">
			<?php
			if ( ! empty( $available_gateways ) ) {
                if(Store_Locator_Manager()) {
                    unset($available_gateways['omise']);
                    unset($available_gateways['omise_paynow']);
                } else {
	...
	<div class="form-row place-order">
		...
		<?php echo apply_filters( 'woocommerce_order_button_html', '<button type="submit" class="btn btn-primary btn-header heading-font" name="woocommerce_checkout_place_order" id="place_order" value="' . esc_attr( $order_button_text ) . '" data-value="' . esc_attr( $order_button_text ) . '">' . esc_html( $order_button_text ) . '</button>' ); // @codingStandardsIgnoreLine ?>
