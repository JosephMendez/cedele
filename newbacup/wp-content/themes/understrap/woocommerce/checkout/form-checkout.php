...
// Remove all cart if exits product for page rewards
if(Rewards_Product_Exits()) {
    foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
        if ( $cart_item['product_id'] != Rewards_Product_Exits() ) {
            WC()->cart->remove_cart_item( $cart_item_key );
        }
    }
}
$_SESSION['buy_membership'] = '';
?>

<form name="checkout" method="post" class="checkout woocommerce-checkout" action="<?php echo esc_url( wc_get_checkout_url() ); ?>" enctype="multipart/form-data">

	<div class="cart-link">
		<i class="fa fa-chevron-left"></i> <a href="<?php echo wc_get_cart_url();?>">Back to shopping cart</a>
	</div>
