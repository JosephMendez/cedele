...
gulp.task( 'watch', function() {
	...
	gulp.watch(
		[
			paths.dev + '/js/pages/*.js',
		],
		gulp.series( 'singleScripts' )
	);
...
gulp.task( 'singleScripts', function() {
	...
	return gulp
		.src( pageSpecificScripts, { allowEmpty: true } )
		.pipe( babel( { presets: ['@babel/preset-env'] } ) )
		.pipe( uglify() )
		.pipe( gulp.dest( paths.js ) );
} );
		...
		.pipe(svgSprite({
			mode: {
				symbol: {
					...
					render: {
						scss: {
							dest:'../../sass/theme/inc/_sprite.scss',
							template: "./sass/theme/inc/_sprite_template.scss"
						}
					}
				}
			}
...
gulp.task( 'compile', gulp.series( 'styles', 'scripts', 'singleScripts', 'dist' ) );
gulp.task( 'build', gulp.series( 'styles', 'scripts', 'singleScripts' ) );
