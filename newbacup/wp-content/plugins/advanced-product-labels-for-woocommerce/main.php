...
class BeRocket_products_label extends BeRocket_Framework {
        ...
        if( ! $options['disable_plabels'] ) {
            ...
            if( ! empty($label_type['label_from_post']) && is_array($label_type['label_from_post']) ) {
                foreach($label_type['label_from_post'] as $label_from_post) {
                    ...
                    if( apply_filters('brapl_set_label_to_product_linked', ! empty($br_label), $br_label, $product) ) {
        ...
        if( ! $options['disable_labels'] ) {
            ...
            foreach($posts_array as $label) {
                ...
                if( $type === TRUE || $type == $br_label['type'] ) {
                    if( apply_filters('brapl_set_label_to_product', ! isset($br_label['data']) || $this->check_label_on_post($label, $br_label['data'], $product), $br_label, $product) ) {
    ...
    public function show_label_on_product($br_label, $product, $label_id = 'demo', $type_of_return = 'echo' ) {
        ...
        foreach($br_label['text'] as $text ) {
            ...
            $html['template-b'] = '<b' . 
                ( empty( $br_label['b_custom_css'] ) ? '' : ' style="' . htmlentities($br_label['b_custom_css']) . '"' ) 
    ...
    public function get_correct_label_style($label_style = '', $br_label, $product) {
        ...
        if( !empty($br_label['font_size']) ) {
            $label_style .= 'font-size:' . $br_label['font_size'] . "{$br_label['font_size_units']};";
        }
