...
if ( $available_providers ) :
	...
	<div class="wc-social-login wc-social-login-link-account">
		<?php foreach ( $available_providers as $provider ) : ?>
			<?php printf( '<a href="%1$s" class="btn btn-shadow btn-social login-%2$s font-body font-weight-semibold"><span class="si si-%2$s"></span>%3$s</a> ', esc_url( $provider->get_auth_url( $return_url ) ), esc_attr( $provider->get_id() ), esc_html( $provider->get_link_button_text() ) ); ?>
		<?php endforeach; ?>
	</div>
	<?php
