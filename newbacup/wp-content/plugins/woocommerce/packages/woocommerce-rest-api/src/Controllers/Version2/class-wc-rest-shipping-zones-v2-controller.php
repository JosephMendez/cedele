...
class WC_REST_Shipping_Zones_V2_Controller extends WC_REST_Shipping_Zones_Controller_Base {
	...
	public function create_item( $request ) {
		...
		if ( ! is_null( $request->get_param( 'toll' ) ) ) {
			$zone->set_zone_toll( $request->get_param( 'toll' ) );
		}

        if ( ! is_null( $request->get_param( 'extra_fee' ) ) ) {
            $zone->set_zone_extra_fee( $request->get_param( 'extra_fee' ) );
        }
