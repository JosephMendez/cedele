...
class WC_Install {
	...
	public static function check_version() {
		self::alter_column_custom();
		self::add_column_extra_fee();
		if ( ! Constants::is_defined( 'IFRAME_REQUEST' ) && version_compare( get_option( 'woocommerce_version' ), WC()->version, '<' ) ) {
			self::install();
			do_action( 'woocommerce_updated' );
		}

		if (version_compare( get_option( 'new_custom_woocommerce_version' ), WC()->version, '<' ) ) {
			self::alter_column_custom();
			self::add_column_extra_fee();
			self::create_table_shipping_time_cost();
			self::create_table_shipping_distance_cost();
			update_option( 'new_custom_woocommerce_version' , WC()->version);
		}
	}

	public function alter_column_custom()
	{
		global $wpdb;
		$result = $wpdb->query("ALTER TABLE `{$wpdb->prefix}woocommerce_shipping_zones` ADD COLUMN IF NOT EXISTS `zone_toll` FLOAT(10,2) NOT NULL DEFAULT '0' AFTER `zone_name`");
	}

    public function add_column_extra_fee()
    {
        global $wpdb;
        $result = $wpdb->query("ALTER TABLE `{$wpdb->prefix}woocommerce_shipping_zones` ADD COLUMN IF NOT EXISTS `zone_extra_fee` FLOAT(10,2) NOT NULL DEFAULT '0' AFTER `zone_toll`");
    }

	public function create_table_shipping_time_cost()
	{
		global $wpdb;
		$sql_query = "CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}woocommerce_shipping_time_cost` 
			(`id` INT(11) NOT NULL AUTO_INCREMENT ,
			`normal_hour_cost` FLOAT(10,2) NOT NULL DEFAULT '0' ,
			`peak_hour_cost` FLOAT(10,2) NOT NULL DEFAULT '0' ,
			`working_day_cost` FLOAT(10,2) NOT NULL DEFAULT '0' ,
			`weekend_cost` FLOAT(10,2) NOT NULL DEFAULT '0' ,
			`occasion_cost` FLOAT(10,2) NOT NULL DEFAULT '0' ,
			PRIMARY KEY (`id`))";
		$result = $wpdb->query($sql_query);
	}

	public function create_table_shipping_distance_cost()
	{
		global $wpdb;
		$sql_query = "CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}woocommerce_shipping_distance_cost` 
			(`id` INT(11) NOT NULL AUTO_INCREMENT ,
			`distance_from` FLOAT(10,2) NOT NULL DEFAULT '0' ,
			`distance_to` FLOAT(10,2) NOT NULL DEFAULT '0' ,
			`distance_cost` FLOAT(10,2) NOT NULL DEFAULT '0' ,
			PRIMARY KEY (`id`))";
		$result = $wpdb->query($sql_query);
	}
...
CREATE TABLE {$wpdb->prefix}woocommerce_shipping_zones (
  ...
  zone_toll float(10,2) NOT NULL DEFAULT '0',
  zone_extra_fee float(10,2) NOT NULL DEFAULT '0',
