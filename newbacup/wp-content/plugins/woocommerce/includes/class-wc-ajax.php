...
class WC_AJAX {
	...
	public static function add_ajax_events() {
		...
		$ajax_events = array(
			...
			'shipping_times_save_changes',
			'shipping_distance_save_changes',
			'shipping_rate_save_changes',
	...
	public static function shipping_zone_add_method() {
		...
		wp_send_json_success(
			array(
				...
				'zone_toll'   => floatval($zone->get_zone_toll()),
				'zone_extra_fee'   => floatval($zone->get_zone_extra_fee()),
	...
	public static function shipping_zone_methods_save_changes() {
		...
		if ( isset( $changes['zone_toll'] ) ) {
			$zone->set_zone_toll( wc_clean( $changes['zone_toll'] ) );
		}

        if ( isset( $changes['zone_extra_fee'] ) ) {
            $zone->set_zone_extra_fee( wc_clean( $changes['zone_extra_fee'] ) );
        }
	...
	public static function shipping_zone_methods_save_settings() {
		...
		wp_send_json_success(
			array(
				'zone_id'   => $zone->get_id(),
				'zone_name' => $zone->get_zone_name(),
				'zone_toll' => $zone->get_zone_toll(),
				'zone_extra_fee' => $zone->get_zone_extra_fee(),
				'methods'   => $zone->get_shipping_methods( false, 'json' ),
	...
	public static function shipping_classes_save_changes() {
		...
		foreach ( $changes as $term_id => $data ) {
			...
			if ( isset( $data['deleted'] ) ) {
				...
				delete_term_meta( $term_id, 'product_shipping_class_fee' );
				continue;
			}
			...
			if ( isset( $data['fee'] ) && floatval($data['fee']) < 0) {
				wp_send_json_error( 'Fee is not valid!' );
				wp_die();
			} else if (isset( $data['fee'] )) {
				$update_args['fee'] = floatval(wc_clean( $data['fee'] ));
			} else {
				$update_args['fee'] = 0;
			}
			...
			if ( isset( $data['newRow'] ) ) {
				...
				if ($term_id) {
					update_term_meta($term_id, 'product_shipping_class_fee', $update_args['fee']);
				}
			} else {
	...
	/**
	 * Handle submissions from assets/js/wc-shipping-time.js
	 */
	public static function shipping_times_save_changes() {
		global $wpdb;
		if ( ! isset( $_POST['wc_shipping_time_nonce'], $_POST['changes'] ) ) {
			wp_send_json_error( 'missing_fields' );
			wp_die();
		}

		if ( ! wp_verify_nonce( wp_unslash( $_POST['wc_shipping_time_nonce'] ), 'wc_shipping_time_nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			wp_send_json_error( 'bad_nonce' );
			wp_die();
		}
		...
		$table_name = $wpdb->prefix . 'woocommerce_shipping_time_cost';
		if (!empty($changes['id'])) {
			$wpdb->update($table_name, $changes, array('id' => $changes['id']));
		} else {
			$wpdb->insert($table_name, $changes);
		}

		wp_send_json_success(
			$wpdb->get_row("SELECT * FROM $table_name")
		);
	}
	
	/**
	 * Handle submissions from assets/js/wc-shipping-distance.js
	 */
	public static function shipping_distance_save_changes() {
		global $wpdb;
		if ( ! wp_verify_nonce( wp_unslash( $_POST['wc_shipping_distance_nonce'] ), 'wc_shipping_distance_nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			wp_send_json_error( 'bad_nonce' );
			wp_die();
		}
		...
		$changes = wp_unslash( $_POST['changes'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		$list_deletes = wp_unslash( $_POST['list_deletes'] );
		$table_name = $wpdb->prefix . 'woocommerce_shipping_distance_cost';

		if (!empty($list_deletes) && is_array($list_deletes)) {
			$delete_in = implode(',', $list_deletes);
			$wpdb->query("DELETE FROM $table_name WHERE id IN ($delete_in)");
		}
	...
	/**
	 * Handle submissions from assets/js/wc-shipping-rate.js
	 */
	public static function shipping_rate_save_changes() {
		global $wpdb;
		if ( ! wp_verify_nonce( wp_unslash( $_POST['wc_shipping_rate_nonce'] ), 'wc_shipping_rate_nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			wp_send_json_error( 'bad_nonce' );
			wp_die();
		}
		...
		$wc_mini_amount = (!isset($_POST['wc_mini_amount']) || floatval($_POST['wc_mini_amount']) < 0) ? 0 :
			wp_unslash( $_POST['wc_mini_amount'] );
		$wc_order_amount_below = (!isset($_POST['wc_order_amount_below']) || floatval($_POST['wc_order_amount_below']) < 0) ? 0 :
			wp_unslash( $_POST['wc_order_amount_below'] );
		if(floatval($wc_order_amount_below) > floatval($wc_mini_amount)) {
            wp_send_json_error( 'Price flat rate can not be greater than value for free shipping!' );
		   return false;
        }
		$wc_apply_shipping_rate = (!isset($_POST['wc_apply_shipping_rate']) || floatval($_POST['wc_apply_shipping_rate']) < 0) ? 0 :
			wp_unslash( $_POST['wc_apply_shipping_rate'] );

		update_option('wc_mini_amount', $wc_mini_amount);
		update_option('wc_order_amount_below', $wc_order_amount_below);
		update_option('wc_apply_shipping_rate', $wc_apply_shipping_rate);

		wp_send_json_success(
			[
				'wc_mini_amount' => $wc_mini_amount,
				'wc_order_amount_below' => $wc_order_amount_below,
				'wc_apply_shipping_rate' => $wc_apply_shipping_rate
			]
		);
	}
