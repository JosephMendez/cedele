...
class WC_Shipping_Zone_Data_Store extends WC_Data_Store_WP implements WC_Shipping_Zone_Data_Store_Interface, WC_Object_Data_Store_Interface {
	...
	public function read( &$zone ) {
		...
		if ( 0 !== $zone->get_id() || '0' !== $zone->get_id() ) {
			$zone_data = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT zone_name, zone_toll,zone_extra_fee, zone_order FROM {$wpdb->prefix}woocommerce_shipping_zones WHERE zone_id = %d LIMIT 1",
					$zone->get_id()
				)
			);
		}
		...
		if ( 0 === $zone->get_id() || '0' === $zone->get_id() ) {
			...
			$zone->set_zone_toll(0);
			$zone->set_zone_extra_fee(0);
			$zone->read_meta_data();
			$zone->set_object_read( true );
			do_action( 'woocommerce_shipping_zone_loaded', $zone );
		} elseif ( $zone_data ) {
			$zone->set_zone_name( $zone_data->zone_name );
			$zone->set_zone_toll( $zone_data->zone_toll );
			$zone->set_zone_extra_fee( $zone_data->zone_extra_fee );
	...
	public function get_zones() {
		global $wpdb;
		return $wpdb->get_results( "SELECT zone_id, zone_name, zone_toll, zone_extra_fee, zone_order FROM {$wpdb->prefix}woocommerce_shipping_zones order by zone_order ASC, zone_id ASC;" );
	}
