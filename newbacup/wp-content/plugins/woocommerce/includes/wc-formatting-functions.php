...
function wc_price( $price, $args = array() ) {
	...
	$priceExplode = explode($args['decimal_separator'], $price);

	$formatted_price = is_admin() && !wp_doing_ajax()
		? ( $negative ? '-' : '' ) . sprintf( $args['price_format'], '<span class="woocommerce-Price-currencySymbol">' . get_woocommerce_currency_symbol( $args['currency'] ) . '</span>', $price )
		: ( $negative ? '-' : '' ) . '<span>' . get_woocommerce_currency_symbol( $args['currency'] ) . $priceExplode[0] . $args['decimal_separator'] . '</span>'.'<span class="price-decima">' . $priceExplode[1] . '</span>';
		$return          = '<span class="woocommerce-Price-amount amount '.(is_admin() ? '' : 'woocommerce-Price-amount-front').'">' . $formatted_price . '</span>';
