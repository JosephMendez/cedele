				...
				<div class="order_data_column_container">
					<div class="order_data_column">
						...
						<p class="form-field form-field-wide wc-order-status">
                            ...
                            <?php
                            $failed_reason = get_post_meta($order->get_id(), 'failed_reason', true);
                            $date_update_status = get_post_meta($order->get_id(), 'last_time_update_status', true);
                            $time = time();
                            $timeSub = $time - strtotime($date_update_status) - 24 * 60 * 60;
                            $status_choices = ['completed', 'failed', 'cancelled'];
                            $status_choice = $order->get_status('edit');
                            $disable = '';
                            $required = '';
                            if ($status_choice == 'failed') {
                                $required = 'required';
                            }
                            if (!empty($date_update_status) && $timeSub >= 0 && in_array($status_choice, $status_choices)) {
                                $disable = 'disabled';
                            }
                            ?>
							<select id="order_status" name="order_status" class="wc-enhanced-select" <?php echo $disable; ?>>
								<?php
								$statuses = wc_get_order_statuses();
								foreach ( $statuses as $status => $status_name ) {
									echo '<option value="' . esc_attr( $status ) . '" ' . selected( $status, 'wc-' . $status_choice, false ) . '>' . esc_html( $status_name ) . '</option>';
								}
								?>
							</select>
						</p>

                        <p class="form-field form-field-wide" id="p-failed-reason" style="<?php if($status_choice != 'failed') echo 'display:none'?>">
                            <label for="order_status">
                                Failed reason:
                            </label>
                            <input type="text" id="failed_reason" name="failed_reason" value="<?php echo $failed_reason?>" <?php echo $disable.' '.$required; ?>/>
                        </p>
