...
<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
	...
	<table class="wc-order-totals">
        ...
        <?php
        $used_redemp_point = get_post_meta($order->get_id(), 'used_redemp_point', true);
        if(isset($used_redemp_point) && $used_redemp_point > 0) {?>
            <tr>
                <td class="label"><?php esc_html_e( 'Use point', 'woocommerce' ); ?>:</td>
                <td width="1%"></td>
                <td class="total">
	...
	<?php if (! empty( $order->get_date_paid() ) ) : ?>
        ...
        <?php
            if (in_array( $order->get_status(), array( 'processing'))) $redemp = 0;
            else $redemp = $used_redemp_point;
        ?>
		<table class="wc-order-totals" style="border-top: 1px solid #999; margin-top:12px; padding-top:12px">
			<tr>
				...
				<td width="1%"></td>
				<td class="total">
					<?php echo wc_price( ($order->get_total() - $redemp), array( 'currency' => $order->get_currency() ) ); // WPCS: XSS ok. ?>
				</td>
			</tr>
		</table>
