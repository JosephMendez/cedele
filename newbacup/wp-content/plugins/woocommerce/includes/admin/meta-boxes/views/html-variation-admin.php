...
<div class="woocommerce_variation wc-metabox closed">
	...
	<div class="woocommerce_variable_attributes wc-metabox-content" style="display: none;">
		<div class="data">
			...
			<div class="custom-product-availability">
				<?php
					global $post;
					...
					$dailyProductAvailableTimeFromVariation = get_post_meta($variation_id, '_daily_product_available_time_from_variation', true);
					$dailyProductAvailableTimeToVariation = get_post_meta($variation_id, '_daily_product_available_time_to_variation', true);
					$checkedDate = get_post_meta($variation_id);
					$_type_variation = get_post_meta($variation_id, '_type', true);
					$oneDayDatePickerVariation = get_post_meta($variation_id, '_one_day_date_picker_variation', true);
					$dateRangeFromVariation = get_post_meta($variation_id, '_date_range_from_variation', true);
					$dateRangeToVariation = get_post_meta($variation_id, '_date_range_to_variation', true);
					$oneDayTimeFromVariation = get_post_meta($variation_id, '_one_day_time_from_variation', true);
					$oneDayTimeToVariation = get_post_meta($variation_id, '_one_day_time_to_variation', true);
					$timeRangeFromVariation = get_post_meta($variation_id, '_time_range_from_variation', true);
					$timeRangeToVariation = get_post_meta($variation_id, '_time_range_to_variation', true);
				?>
				<input type="hidden" name="_old_product_title" value="<?php echo $post->post_title; ?>">
				<?php
				$tproduct = wc_get_product($post->ID);
				$children_ids = $tproduct->get_children();
				if ($children_ids):
				foreach ($children_ids as $vp_key => $vp_id):
					$vp_object = new WC_Product_Variation($vp_id);
					$old_price = '';
					if ($vp_object) {
						$old_price = $vp_object->regular_price;
					}
				?>
				<input type="hidden" name="_old_variant_product_price[<?php echo $vp_id ?>]" value="<?php echo $old_price; ?>">
				<?php endforeach;endif; ?>
				<div class="product-availability-variations[<?php echo esc_attr( $loop ) ?>]" style="display:flex">
					<div class="product-availability-title-variations[<?php echo esc_attr( $loop ) ?>]">
						Product availability
					</div>
					<div style="display:flex;margin-left:200px" class="product-availability-type-product-variations[<?php echo esc_attr( $loop ) ?>]">
						<div class="product-availability-daily-product-variations[<?php echo esc_attr( $loop ) ?>]">
							<input id="daily-product-variations[<?php echo esc_attr( $loop ) ?>]" type="radio" value="daily-product-variations[<?php echo esc_attr( $loop ) ?>]" name="product-avalability-type-variations[<?php echo esc_attr( $loop ) ?>]" data-loop="<?php echo esc_attr( $loop ) ?>"/>
							<label>Normal Product</label>
						</div>
						<div style="margin-left:10px" class="product-availability-season-product-variations[<?php echo esc_attr( $loop ) ?>]">
							<input id="season-product-variations[<?php echo esc_attr( $loop ) ?>]" type="radio" name="product-avalability-type-variations[<?php echo esc_attr( $loop ) ?>]" value="season-product-variations[<?php echo esc_attr( $loop ) ?>]" data-loop="<?php echo esc_attr( $loop ) ?>"/>
							<label>Seasonal Product</label>
						</div>
					</div>
				</div>
				<input type="hidden" id="type-choosen-variation[<?php echo esc_attr( $loop ) ?>]" name="type-choosen-variation[<?php echo esc_attr( $loop ) ?>]" value="<?php echo $_type_variation ?>"/>
				<div class="daily-product-info-variations[<?php echo esc_attr( $loop ) ?>]" id="daily-product-info-variations[<?php echo esc_attr( $loop ) ?>]">
					<div class="daily-product-available-variations[<?php echo esc_attr( $loop ) ?>]">
						<label style="font-weight:bold">Available day(s):</label>
						<div class="daily-product-list-date-available-variations[<?php echo esc_attr( $loop ) ?>]" style="display:flex; margin-top:10px">
							<?php
								foreach($listDay as $key => $value) {
									if ($checkedDate[$key] == NULL) {
										echo '<div>
										<input type="checkbox" checked value="' .$key. '" name="' .$key.'['.$loop.']'. '"/>&nbsp;
											<label>' . $value . '</label>
										</div>&nbsp;&nbsp;';
									} else {
										echo '<div>
										<input type="checkbox" value="' .$key. '" name="' .$key.'['.$loop.']'. '"';
										?>
										<?php
					...
					<div class="daily-product-available-variation[<?php echo esc_attr( $loop ) ?>]" style="margin-top:30px">
						<label style="font-weight:bold">Available time:</label>
						<div class="daily-product-available-time-variation[<?php echo esc_attr( $loop ) ?>]" style="margin-top:15px; display:flex">
							<div class="daily-product-available-time-from-variation[<?php echo esc_attr( $loop ) ?>]" style="display:flex;align-items:center">
								<label>From: </label>
								<input name="daily-product-available-time-from-variation[<?php echo esc_attr( $loop ) ?>]" type="time" value="<?php echo $dailyProductAvailableTimeFromVariation !== '' ? $dailyProductAvailableTimeFromVariation : '00:00' ?>"/>
							</div>
							<div class="daily-product-available-time-to-variation[<?php echo esc_attr( $loop ) ?>]" style="margin-left:30px">
								<label>To:</label>
								<input name="daily-product-available-time-to-variation[<?php echo esc_attr( $loop ) ?>]" type="time" value="<?php echo $dailyProductAvailableTimeToVariation !== '' ? $dailyProductAvailableTimeToVariation : '23:59' ?>" />
							</div>
						</div>
					</div>
				</div>

				<div class="season-product-info-variations[<?php echo esc_attr( $loop ) ?>] season-product-info-variations" id="season-product-info-variations[<?php echo esc_attr( $loop ) ?>]" style="display:none">
					<div class="season-product-available-variations[<?php echo esc_attr( $loop ) ?>]" style="display:flex; align-items:center">
						<label style="font-weight:bold;margin-right:20px">Available date(s):</label>
						<div class="season-product-list-date-available-variations[<?php echo esc_attr( $loop ) ?>]" style="display:flex">
							<select data-loop="<?php echo esc_attr( $loop ) ?>" id="season-product-list-date-available-select-variations[<?php echo esc_attr( $loop ) ?>]" name="season-product-list-date-available-select-variations[<?php echo esc_attr( $loop ) ?>]" style="margin-left:10px">
								<option value="one-day-only-variations" name="one-day-only-variations[<?php echo esc_attr( $loop ) ?>]"> One Day Only </option>
								<option value="time-range-variations" name="time-rang-variations[<?php echo esc_attr( $loop ) ?>]e"> Time Range </option>
							</select>
							<input id="one-day-date-picker-variations[<?php echo esc_attr( $loop ) ?>]" type="date" value="<?php echo $oneDayDatePickerVariation !== '' ? $oneDayDatePickerVariation : '' ?>" name="one-day-date-picker-variations[<?php echo esc_attr( $loop ) ?>]" style="margin-left:10px;">
							<div class="date-range-from-variations[<?php echo esc_attr( $loop ) ?>]" style="display:flex;align-items: center;margin-left:20px">
								<label for="date-range-from">From:</label>
								<input type="date" id="date-range-from-variations[<?php echo esc_attr( $loop ) ?>]" value="<?php echo $dateRangeFromVariation !== '' ? $dateRangeFromVariation : '' ?>" name="date-range-from-variations[<?php echo esc_attr( $loop ) ?>]" style="margin-left:10px">
							</div>
							<div class="date-range-to-variations[<?php echo esc_attr( $loop ) ?>]" style="display:flex;align-items: center;margin-left:20px">
								<label for="date-range-to">To:</label>
								<input type="date" id="date-range-to-variations[<?php echo esc_attr( $loop ) ?>]" value="<?php echo $dateRangeToVariation !== '' ? $dateRangeToVariation : '' ?>" name="date-range-to-variations[<?php echo esc_attr( $loop ) ?>]" style="margin-left:10px">
							</div>
						</div>
					</div>

					<div class="season-product-available-one-day-variations[<?php echo esc_attr( $loop ) ?>]" style="margin-top:30px;">
						<label style="font-weight:bold">Available time:</label>
						<div class="season-product-available-time-variations[<?php echo esc_attr( $loop ) ?>]" style="margin-top:15px; display:flex">
							<div class="season-product-available-time-from-variations[<?php echo esc_attr( $loop ) ?>]" style="display:flex;align-items:center">
								<label>From: </label>
								<input name="one-day-time-from-variations[<?php echo esc_attr( $loop ) ?>]" type="time" value="<?php echo $oneDayTimeFromVariation !== '' ? $oneDayTimeFromVariation : '00:00' ?>"/>
							</div>
							<div class="season-product-available-time-to-variations[<?php echo esc_attr( $loop ) ?>]" style="margin-left:30px">
								<label>To:</label>
								<input name="one-day-time-to-variations[<?php echo esc_attr( $loop ) ?>]" type="time" value="<?php echo $oneDayTimeToVariation !== '' ? $oneDayTimeToVariation : '23:59' ?>"/>
							</div>
						</div>
					</div>

					<div class="season-product-available-time-range-variations[<?php echo esc_attr( $loop ) ?>]" style="margin-top:30px">
						<label style="font-weight:bold">Available time:</label>
						<div class="season-product-available-time-variations[<?php echo esc_attr( $loop ) ?>]" style="margin-top:15px; display:flex">
							<div class="season-product-available-time-from-variations[<?php echo esc_attr( $loop ) ?>]" style="display:flex;align-items:center">
								<label>From: </label>
								<input name="time-range-from-variations[<?php echo esc_attr( $loop ) ?>]" type="time" value="<?php echo $timeRangeFromVariation !== '' ? $timeRangeFromVariation : '00:00' ?>"/>
							</div>
							<div class="season-product-available-time-to-variations[<?php echo esc_attr( $loop ) ?>]" style="margin-left:30px">
								<label>To:</label>
								<input name="time-range-to-variations[<?php echo esc_attr( $loop ) ?>]" type="time" value="<?php echo $timeRangeToVariation !== '' ? $timeRangeToVariation : '23:59' ?>"/>
							</div>
						</div>
					</div>
