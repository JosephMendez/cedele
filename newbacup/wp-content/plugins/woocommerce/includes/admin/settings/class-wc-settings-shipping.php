...
class WC_Settings_Shipping extends WC_Settings_Page {
	...
	public function get_sections() {
		$sections = array(
			...
			'shipping_time' => __( 'Shipping time', 'woocommerce' ),
			'shipping_distance' => __( 'Distance', 'woocommerce' ),
			'shipping_rate' => __( 'Shipping rate', 'woocommerce' ),
		);
	...
	public function output() {
		...
		} elseif ('shipping_time' === $current_section) {
			$hide_save_button = true;
			$this->output_shipping_cost_time();
		} elseif ('shipping_distance' === $current_section) {
			$hide_save_button = true;
			$this->output_shipping_cost_distance();
		} elseif ('shipping_rate' === $current_section) {
			$hide_save_button = true;
			$this->output_shipping_rate();
		} else {
	...
	protected function output_shipping_class_screen() {
		...
		$shipping_class_columns = apply_filters(
			...
			array(
				...
				'wc-shipping-class-fee'         => __( 'Fee', 'woocommerce' ),
	...
	/**
	 * Handles output of the shipping time settings screen.
	 */
	protected function output_shipping_cost_time() {
		
		global $wpdb;
		$result = $wpdb->get_row("SELECT * FROM `{$wpdb->prefix}woocommerce_shipping_time_cost`", ARRAY_A);
		$result = !empty($result) ? $result : [];
		wp_localize_script(
			'wc-shipping-time',
			'shippingTimeLocalizeScript',
			array(
				'url'                    => admin_url('admin-ajax.php'),
				'data'                   => $result,
				'id'                     => !empty($result["id"]) ? $result["id"] : 0,
				'wc_shipping_time_nonce' => wp_create_nonce( 'wc_shipping_time_nonce' ),
			)
		);
		wp_enqueue_script( 'wc-shipping-time' );

		include_once dirname( __FILE__ ) . '/views/html-admin-page-shipping-time.php';
	}

	/**
	 * Handles output of the shipping distance settings screen.
	 */
	protected function output_shipping_cost_distance() {
		
		global $wpdb;
		$result = $wpdb->get_results("SELECT * FROM `{$wpdb->prefix}woocommerce_shipping_distance_cost`", ARRAY_A);
		$result = !empty($result) ? $result : [];
		wp_localize_script(
			'wc-shipping-distance',
			'shippingDistanceLocalizeScript',
			array(
				'url'                        => admin_url('admin-ajax.php'),
				'editLabel'                  => __( 'Edit', 'woocommerce' ),
				'removeLabel'                => __( 'Remove', 'woocommerce' ),
				'data'                       => $result,
				'wc_shipping_distance_nonce' => wp_create_nonce( 'wc_shipping_distance_nonce' ),
			)
		);
		wp_enqueue_script( 'wc-shipping-distance' );

		include_once dirname( __FILE__ ) . '/views/html-admin-page-shipping-distance.php';
	}

	/**
	 * Handles output of the shipping rate settings screen.
	 */
	protected function output_shipping_rate() {
		...
		wp_localize_script(
			...
			array(
				...
				'wc_shipping_rate_nonce' => wp_create_nonce( 'wc_shipping_rate_nonce' ),
			)
		);
		wp_enqueue_script( 'wc-shipping-rate' );

		include_once dirname( __FILE__ ) . '/views/html-admin-page-shipping-rate.php';
	}
}
