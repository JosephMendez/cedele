...
<table class="form-table wc-shipping-zone-settings">
	<tbody>
		...
		<tr valign="top" class="">
			<th scope="row" class="titledesc">
				<label for="zone_toll">
					<?php esc_html_e( 'Tolls', 'woocommerce' ); ?>
					<?php echo wc_help_tip( __( 'Input for calculating Tolls fee.', 'woocommerce' ) ); // @codingStandardsIgnoreLine ?>
				</label>
			</th>
			<td class="forminp">
				<input type="number" data-attribute="zone_toll" name="zone_toll" id="zone_toll" step="0.01" value="<?php echo esc_attr( $zone->get_zone_toll( 'edit' ) ); ?>" placeholder="<?php esc_attr_e( 'Zone tolls', 'woocommerce' ); ?>"> <span style="line-height: 30px;">$</span>
			</td>
		</tr>
        <tr valign="top" class="">
            <th scope="row" class="titledesc">
                <label for="zone_extra_fee">
                    <?php esc_html_e( 'Tolls extra fee', 'woocommerce' ); ?>
                    <?php echo wc_help_tip( __( 'Input for calculating Tolls fee.', 'woocommerce' ) ); // @codingStandardsIgnoreLine ?>
                </label>
            </th>
            <td class="forminp">
                <input type="number" data-attribute="zone_extra_fee" name="zone_extra_fee" id="zone_extra_fee" step="0.01" value="<?php echo esc_attr( $zone->get_zone_extra_fee( 'edit' ) ); ?>" placeholder="<?php esc_attr_e( 'Zone tolls', 'woocommerce' ); ?>"> <span style="line-height: 30px;">$</span>
            </td>
        </tr>
	</tbody>
</table>
