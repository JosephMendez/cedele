		...
		foreach ( $shipping_class_columns as $class => $heading ) {
			...
			switch ( $class ) {
				...
				case 'wc-shipping-class-description':
					...
					<?php
					break;
				case 'wc-shipping-class-fee':
					?>
					<div class="view">{{ data.fee }}</div>
					<div class="edit"><input type="number" min="0" pattern="[0-9\.]{0,}" name="fee[{{ data.term_id }}]" data-attribute="fee" value="{{ data.fee }}" placeholder="<?php esc_attr_e( 'Fee', 'woocommerce' ); ?>" /></div>
					<?php
					break;
