...
( function( $, data, wp, ajaxurl ) {
	$( function() {
		$('body').on('blur', 'input[name="zone_toll"]', function(event) {
			let value = parseFloat($(this).val()) ? parseFloat($(this).val()) : 0
			$(this).val(value);
		});
        $('body').on('blur', 'input[name="zone_extra_fee"]', function(event) {
            let value = parseFloat($(this).val()) ? parseFloat($(this).val()) : 0
            $(this).val(value);
        });
		var $table          = $( '.wc-shipping-zone-methods' ),
			...
			ShippingMethod       = Backbone.Model.extend({
				...
				logChanges: function( changedRows ) {
					...
					if ( typeof changedRows.zone_toll !== 'undefined' ) {
						changes.zone_toll = changedRows.zone_toll;
					}

                    if ( typeof changedRows.zone_extra_fee !== 'undefined' ) {
                        changes.zone_extra_fee = changedRows.zone_extra_fee;
                    }
			...
			ShippingMethodView = Backbone.View.extend({
				...
				initialize: function() {
					...
					$( document.body ).on(
						'input change',
						'#zone_name, #zone_locations, #zone_postcodes, #zone_toll, #zone_extra_fee',
						{ view: this },
						this.onUpdateZone
					);
				...
				render: function() {
					var methods     = _.indexBy( this.model.get( 'methods' ), 'instance_id' ),
						...
						zone_toll   = this.model.get( 'zone_toll' ),
						zone_extra_fee   = this.model.get( 'zone_extra_fee' ),
						view        = this;

					// Set name.
					$('.wc-shipping-zone-name').text( zone_name ? zone_name : data.strings.default_zone_name );
					$('.wc-shipping-zone-toll').text( zone_toll ? zone_toll : 0 );
					$('.wc-shipping-zone-extra-fee').text( zone_extra_fee ? zone_extra_fee : 0 );
			...
			shippingMethod = new ShippingMethod({
				...
				zone_toll: data.zone_toll,
				zone_extra_fee: data.zone_extra_fee
			} ),
