...
class Advance_Ecommerce_Tracking
{
    ...
    private function define_admin_hooks()
    {
        ...
        $this->loader->add_action( 'wp_ajax_aet_fetch_data', $plugin_admin, 'aet_fetch_data' );
    }
    ...
    private function define_public_hooks()
    {
        ...
        if ( !is_admin() ) {
            ...
            if ( $ua ) {
                ...
                if ( !empty($aet_et_tracking_settings) ) {
            ...
            $pi = get_option( 'selected_data_ua_ft' );
            
            if ( $pi ) {
                $aet_ft_tracking_settings = json_decode( get_option( 'aet_ft_tracking_settings' ), true );
                
                if ( !empty($aet_ft_tracking_settings) ) {
                    $aft_enable = $aet_ft_tracking_settings['ft_enable'];
                    $fb_ecommerce_tracking = $aet_ft_tracking_settings['fb_ecommerce_tracking'];
                } else {
                    $aft_enable = 'off';
            ...
            if ( 'on' === $aft_enable ) {
                $this->loader->add_action( 'wp_head', $plugin_public, 'aet_add_ft_tracking_code' );
                
                if ( 'on' === $fb_ecommerce_tracking ) {
                    $this->loader->add_action( 'woocommerce_after_checkout_form', $plugin_public, 'aet_ft_checkout_process' );
                    $this->loader->add_action(
                        'woocommerce_thankyou',
                        $plugin_public,
                        'aet_ft_order_pro_comp',
                        10
                    );
                }
