<?php
/*
  Plugin Name: Cedele WooCommerce Products Filter
  Plugin URI: 
...
//                        );
    ...
    public function woof_products($atts, $is_prediction = false) {
        ...
        if (empty($product_ids)) {
            ...
            if ($post__in) {
                $args['post__in'] = explode(",", array_diff($post__in, $exclude_ids));
            }
        } else {
            $args['post__in'] = explode(",", array_diff($product_ids, $exclude_ids));
        }

        if ($atts['filter_by'] == 'sale') {
            $sale_products = wc_get_product_ids_on_sale();
            if (count($sale_products) > 0){
                $args['post__in'] = array_diff($sale_products, $exclude_ids);
            } else {
                $args['post__in'] = array(0);
            }
        } 
        if ($atts['filter_by'] == 'popular') {
            ...
            if (count($popular_posts->posts) > 0){
                $args['post__in'] = array_diff($popular_posts->posts, $exclude_ids);
            } else {
                $args['post__in'] = array(0);
            }
        }
        if ($atts['filter_by'] == 'new') {
            ...
            if (count($new_posts->posts) > 0){
                $args['post__in'] = array_diff($new_posts->posts, $exclude_ids);
            } else {
                $args['post__in'] = array(0);
            }
        }
