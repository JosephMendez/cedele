...
function removeFilterFromShortCode(shortcode) {
  var shortcodeArr = shortcode.split(' ');
  for (var i=0; i<shortcodeArr.length; i++){
    if (shortcodeArr[i].startsWith('filter_by')){
...
function woof_init_filter() {
  jQuery('body').on('click', '.woocommerce-filter-product .filter-option', function() {
    jQuery('.woocommerce-filter-product .filter-option').removeClass('selected');
    jQuery(this).addClass('selected');
    if (!jQuery("#is_woo_shortcode").length) {
      var shortcode = jQuery('#woof_results_by_ajax').attr('data-shortcode');
      shortcode = removeFilterFromShortCode(shortcode);
      jQuery('#woof_results_by_ajax').attr('data-shortcode', shortcode + ' filter_by=' + jQuery(this).attr('data-filter-type'));
      woof_ajax_page_num = 1;
      woof_submit_link(woof_get_submit_link(), 0);
      return false;
    }
  });
  var filterBy = getUrlParameter('filter_by');
  if (filterBy){
    jQuery('.woocommerce-filter-product .filter-option').removeClass('selected');
    jQuery('.woocommerce-filter-product .filter-option[data-filter-type="'+filterBy+'"]').addClass('selected');
  }
}
...
function woof_submit_link(link, ajax_redraw) {
  ...
  if (woof_is_ajax === 1 && !ajax_redraw) {
    ...
    jQuery.post(woof_ajaxurl, data, function(content) {
      ...
      var shortcodes = jQuery('#woof_results_by_ajax').attr('data-shortcode').split(' ');
      shortcodes.forEach(function(sc){
        if (sc.includes('filter_by')){
          var filterBy = sc.split('=')[1];
          jQuery('.woocommerce-filter-product .filter-option').removeClass('selected');
          jQuery('.woocommerce-filter-product .filter-option[data-filter-type="'+filterBy+'"]').addClass('selected');
        }
      });
...
function woof_get_submit_link() {
  ...
  var filterByElm = jQuery('.woocommerce-filter-product .filter-option.selected');
  if (filterByElm.length){
    var filterBy = jQuery(filterByElm).attr('data-filter-type');
    var href = new URL(link);
    href.searchParams.set('filter_by', filterBy);
    link = href.toString();
  }
