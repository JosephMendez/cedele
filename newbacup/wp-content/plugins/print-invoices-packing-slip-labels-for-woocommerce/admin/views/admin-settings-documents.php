...
<div class="wf-tab-content" data-id="<?php echo $target_id;?>">
	...
	<p class="wt_info_box">
		...
		<br /><br />
		<i><?php _e('Note: Also make sure to fill-up the Company and Address information from the General tab above prior to customization of the respective document types.','print-invoices-packing-slip-labels-for-woocommerce');?></i>
	</p>
	  	...
	  	foreach ($wt_pklist_common_modules_main as $wt_pklist_common_modules_sub)
	  	{
		    ...
		    foreach($wt_pklist_common_modules_sub as $k=>$v)
		    {
		    	...
		    	if(isset(Wf_Woocommerce_Packing_List_Public::$modules_label[$k]))
		    	{
		    		...
		    		$settings_url=admin_url('admin.php?page=wf_woocommerce_packing_list_document_settings_page#');
		    		if($k=='invoice')
		    		{
		    			$settings_url.=$module_id;
		    		}else
		    		{
		    			$settings_url.='wf-other-documents';
		    		}
